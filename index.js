...
const fetchAndStoreDbDocuments = async (dbName) => {
    ...
    for (const row of rows) {
        ...
        const folderPath = path.join(process.cwd(), dbName, `${numKeys}-key-value-pairs`, docType);
...
getAllDatabasesAndDocuments().then(() => {
    ...
    app.get('/api/medic', (req, res) => {
        ...
        const dirs = fs.readdirSync(medicDir, {withFileTypes: true})
            .filter(dirent => dirent.isDirectory() && parseInt(dirent.name.split('-')[0]) > 10)
        ...
        dirs.forEach(dir => {
            const subDirs = fs.readdirSync(path.join(medicDir, dir), {withFileTypes: true})
            ...
            subDirs.forEach(subDir => {
                const files = fs.readdirSync(path.join(medicDir, dir, subDir))
                    .filter(file => file.endsWith('.json'));
                console.log("Files: ", files, "\n");
                files.forEach(file => {
                    const data = JSON.parse(fs.readFileSync(path.join(medicDir, dir, subDir, file), 'utf8'));
